...
update_track() {
  ...
  if [[ $PLAYER_STATE = "Playing" ]]; then
    ...
    sketchybar  --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes icon.padding_right=3 icon.color=$GREEN

  elif [[ $PLAYER_STATE = "Paused" ]]; then
    sketchybar --set $NAME icon.color=$YELLOW
  elif [[ $PLAYER_STATE = "Stopped" ]]; then
    sketchybar --set $NAME icon.color=$YELLOW label.drawing=no icon.padding_right=7
  else
    sketchybar --set $NAME icon.color=$YELLOW
  fi
}
